...
<template>
  <div class="page">
    ...
    <div class="surface" style="padding:16px">
      ...
      <el-table :data="rows" v-loading="loading" style="width:100%" empty-text="暂无路由规则">
        <el-table-column prop="alias" label="别名" width="120">
          <template #default="{ row }"><el-tag effect="dark" round>{{ row.alias }}</el-tag></template>
        </el-table-column>
          ...
          <template #default="{ row }">
            ...
            <span v-else class="chain">
              <template v-for="(f, i) in row.fallbacks" :key="i">
                ...
                <el-tag size="small" type="info" effect="plain" class="mono">{{ f.provider }}:{{ f.model }}</el-tag>
              </template>
            </span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.mode === 'create' ? '新增路由规则' : '编辑路由规则'" width="640px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="110px">
        <el-form-item label="别名" prop="alias">
          <el-input v-model="form.alias" placeholder="auto / cheap / smart" :disabled="dialog.mode === 'edit'" />
        </el-form-item>

        <el-divider content-position="left">首选目标</el-divider>
        <div class="row2">
          <el-form-item label="供应商" prop="primaryProvider" class="col">
            <el-select v-model="form.primaryProvider" placeholder="选择" filterable allow-create style="width:100%">
        ...
        <div class="row2">
          <el-form-item label="阈值 Token" class="col">
            <el-input-number v-model="form.maxPromptTokens" :min="0" :step="500" controls-position="right" style="width:100%" placeholder="留空不升级" />
          </el-form-item>
          <el-form-item label="升级供应商" class="col">
            <el-select v-model="form.escalateProvider" placeholder="选择" filterable allow-create clearable style="width:100%">
              <el-option v-for="p in providers" :key="p" :label="p" :value="p" />
            </el-select>
          </el-form-item>
        </div>
        ...
        <el-divider content-position="left">
          降级链
          <el-button link type="primary" @click="addFallback"><el-icon><Plus /></el-icon>添加</el-button>
        </el-divider>
        <div v-if="!form.fallbacks.length" class="muted" style="padding:4px 0 8px">暂无降级目标(首选失败时无兜底)</div>
        <div v-for="(f, i) in form.fallbacks" :key="i" class="fb-row">
          <span class="fb-seq">{{ i + 1 }}</span>
          <el-select v-model="f.provider" placeholder="供应商" filterable allow-create style="width:160px">
            <el-option v-for="p in providers" :key="p" :label="p" :value="p" />
          </el-select>
          <el-input v-model="f.model" placeholder="物理模型" style="flex:1" />
          <el-button link type="danger" @click="removeFallback(i)"><el-icon><Delete /></el-icon></el-button>
        </div>
      </el-form>
      <template #footer>
...
.chain { display: inline-flex; align-items: center; flex-wrap: wrap; gap: 6px; }
.chain-arrow { color: var(--app-text-secondary); }
.row2 { display: flex; gap: 16px; }
.row2 .col { flex: 1; }
.fb-row { display: flex; align-items: center; gap: 10px; margin-bottom: 10px; }
.fb-seq {
  width: 24px; height: 24px; flex: none;
  border-radius: 50%; background: var(--el-color-primary-light-9); color: var(--el-color-primary-dark-2);
