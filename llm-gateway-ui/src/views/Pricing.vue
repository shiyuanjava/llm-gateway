...
<template>
  <div class="page">
    ...
    <div class="surface" style="padding:16px">
      <div class="toolbar">
        <el-button type="primary" @click="openCreate"><el-icon><Plus /></el-icon>&nbsp;新增单价</el-button>
      ...
      <el-table :data="rows" v-loading="loading" style="width:100%" empty-text="暂无计费数据">
        ...
        <el-table-column label="输入 / 1K (USD)" width="180" align="right">
          <template #default="{ row }"><span class="tabular-nums">${{ Number(row.inputPer1k).toFixed(5) }}</span></template>
        </el-table-column>
        <el-table-column label="输出 / 1K (USD)" width="180" align="right">
          <template #default="{ row }"><span class="tabular-nums">${{ Number(row.outputPer1k).toFixed(5) }}</span></template>
        </el-table-column>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.mode === 'create' ? '新增计费' : '编辑计费'" width="480px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="130px">
        ...
        <el-form-item label="输入 / 1K (USD)">
          <el-input-number v-model="form.inputPer1k" :precision="5" :step="0.0001" :min="0" controls-position="right" style="width:100%" />
        </el-form-item>
        <el-form-item label="输出 / 1K (USD)">
          <el-input-number v-model="form.outputPer1k" :precision="5" :step="0.0001" :min="0" controls-position="right" style="width:100%" />
        </el-form-item>
        <el-form-item label="缓存读 / 1K (USD)">
          <el-input-number v-model="form.cacheReadPer1k" :precision="5" :step="0.0001" :min="0" :value-on-clear="null" controls-position="right" style="width:100%" placeholder="留空按输入单价" />
        </el-form-item>
        <el-form-item label="缓存写 / 1K (USD)">
          <el-input-number v-model="form.cacheWritePer1k" :precision="5" :step="0.0001" :min="0" :value-on-clear="null" controls-position="right" style="width:100%" placeholder="留空按输入单价" />
        </el-form-item>
      </el-form>
      <template #footer>
