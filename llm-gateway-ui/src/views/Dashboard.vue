  ...
  <div class="page">
    ...
    <div class="surface table-wrap">
      <div class="table-head">
        租户用量明细
        <el-tag v-if="truncated" size="small" type="warning" style="margin-left: 8px"
          >仅显示 Token 用量前 {{ rows.length }} 个租户</el-tag
        >
      </div>
      <el-table
        :data="rows"
      ...
      >
        ...
        <el-table-column prop="requests" label="请求数" width="120" align="right">
          <template #default="{ row }"
            ><span class="tabular-nums">{{ fmtInt(row.requests) }}</span></template
          >
        </el-table-column>
        <el-table-column prop="cacheHits" label="缓存命中" width="110" align="right">
          <template #default="{ row }"
            ><span class="tabular-nums">{{ fmtInt(row.cacheHits) }}</span></template
          >
        </el-table-column>
