...
| 5050 | Container Registry | 构建/部署都在本机时可不对公网开放；若开放，尽快上 HTTPS |
...
```ruby
registry_external_url 'http://119.29.120.205:5050'
```
...
kubectl -n llm-gateway create secret docker-registry gitlab-registry \
  --docker-server=119.29.120.205:5050 \
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: order-service
          image: 119.29.120.205:5050/<命名空间>/llm-gateway/order-service:commit-sha
          ports:
            - containerPort: 8080
          readinessProbe:
            httpGet:
